...
class DecryptionFunctionality implements Functionality
{
    public function modify(string $input, SplFileObject $stream): void
    {
        ...
        try{
                        ...
                        if (array_key_exists($input, $assocoateLineKey)) {
                            ...
                            foreach ($assocoateLineKey as $key => $value) {
                                if ($key !== $input) {
                                    ...
                                    if (is_string($value)) {
                                        if (!openssl_private_decrypt(base64_decode($value), $decryptedValue, $privateKey)) {
                                            throw new \Exception("Decryption failed for value: " . openssl_error_string());
                                        }
                                        $tempStream->fwrite($decryptedValue . ",");
                                    } else {
            ...
            $privateKey = null; // Clear the private key from memory
